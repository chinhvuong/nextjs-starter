...
import { selectWallet, selectIsWalletConnected, selectWalletLoading } from '@/store/slices/wallet/selectors';
import { connectWalletStart } from '@/store/slices/wallet/walletSlice';
import { Button } from '@/shared/components/button';
import { Card, CardContent, CardHeader, CardTitle } from '@/shared/components/card';

export const BorrowPageRedux: React.FC = () => {
  const dispatch = useAppDispatch();
  const wallet = useAppSelector(selectWallet);
  const isConnected = useAppSelector(selectIsWalletConnected);
  const isConnecting = useAppSelector(selectWalletLoading);

  const handleConnectWallet = () => {
    dispatch(connectWalletStart());
    // Here you would typically call the actual wallet connection logic
  };
